...
# Point Python at python-service/app so imports find app modules (data_helper, main, etc.)
_project_root = Path(__file__).resolve().parent
_app_dir = _project_root / "python-service" / "app"
if str(_app_dir) not in sys.path:
    sys.path.insert(0, str(_app_dir))
# Use app dir as working directory so relative paths work (test.db, token.json, credentials.json)
os.chdir(_app_dir)
...
def _demo_emails_path() -> Path:
    path = _app_dir / "reports" / "demo_emails.json"
