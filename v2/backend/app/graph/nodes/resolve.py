...
Order of trust: sender domain > known-vendor mention in the body > phrase
match. Payment processors are a special case: paypal.com really did send the
mail, but the merchant is inside the body, so the domain only wins as a
fallback.
"""
...
from .. import vendors
from ..state import ReceiptState, missing_fields
...
async def resolve(state: ReceiptState) -> dict:
    ...
    vendor = None
    source = None
    if domain_vendor and not is_processor:
